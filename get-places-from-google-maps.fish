...
set url "https://maps.googleapis.com/maps/api/place/nearbysearch/json\
?key=$GOOGLE_MAPS_API_KEY\
&location=$LATITUDE%2C$LONGITUDE&radius=5000\
&type=$GOOGLE_PLACE_TYPE"

curl -s $url > tmp/places-from-google-maps-1.json
...
while test $next_page_token != "null"
    ...
    sleep 2
    curl -s "$url&pagetoken=$next_page_token" > tmp/places-from-google-maps-$page.json
